...
def has_cycle(head: ListNode) -> Optional[ListNode]:
    # setup fast and slow iterators
    fast = slow = head

    while fast and fast.next:
        # fast walks two steps at a time while slow walks one step at a time
        slow, fast = slow.next, fast.next.next

        # there is a cycle
        if slow is fast:

            # reset only slow to the start of the linked list
            slow = head

            # now iterators walk in tandem
            while slow is not fast:
                slow, fast = slow.next, fast.next

            # eventually they meet at the start of the cycle...
            return slow

    # or, if there is no cycle, it returns None
    return None
