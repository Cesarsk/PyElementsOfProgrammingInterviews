...
def minimum_path_weight(triangle: List[List[int]]) -> int:
    ...
    for row in triangle:  # loop every row in triangle
        # the array is overwritten for every row considering the elements of the previous row.

        # loop for j to row with j < row
        min_weight_to_curr_row = [
            row[j] +
            min(  # can't understand max(j-1,0) and min(j, len(min_weight)-1
